...
- Use `browser_snapshot` for normal page inspection and interaction, and `browser_fetch` for authenticated APIs where browser cookies, Origin, CORS, and the browser TLS stack matter. For large responses, binary downloads, or requests that should not run under page CORS/CSP, use `browser_curl` or `browser-control curl`. It invokes the real curl with a temporary browser cookie jar plus User-Agent, Origin, and Referer derived from the source tab. Ordinary successful HTML responses are projected to a bounded readable summary so raw markup does not flood the model context; use `-o <path>`/`--output <path>` when you explicitly need the full HTML or an unrestricted download. Explicit header, write-out, and raw-output flags preserve curl's original stream. MCP body responses are capped at 8 MiB.
...
- Use MCP `browser_click` or CLI `browser-control click`; never hand-write JavaScript to click an element.
- Preferred MCP flow: call `browser_snapshot`, then pass the exact accessible name and role, e.g. `browser_click({"element":"Sign in","role":"button"})`.
- Preferred CLI form: `browser-control click -b brave/work "Sign in" --role button`. A unique visible name works without `--role`; use `--selector` only as a fallback.
- Semantic clicks use exact matching by default and refuse ambiguous elements. Capture a fresh snapshot and add `role`, or narrow the selector, instead of guessing. CLI clicks likewise refuse to choose among multiple open pages; address a named tab or pass `--target <url-regex>`.
- Playwright performs the real input action, including actionability checks, scrolling, trusted pointer events, and navigation waiting. Do not replace it with `element.click()` through eval.
